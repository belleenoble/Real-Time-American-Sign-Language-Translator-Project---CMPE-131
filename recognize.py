...
# # no hand framses accounted for in a 1 letter translation
...
#     #move the write landmarks to designated origin
...
#     #make the data independent of hand size and distance from the camera (for inclusivity)
...
# option = HandLanderOptions(
#     base_options = BaseOptions(model_asset_path=MODEL_PATH),
#     running_mode=RunningMode.VIDEO,
#     num_hands=1,
#     min_hand_dectection_confidence=0.5,
...
# #recognition (word building state) variables
# recent_predictions = []
# committed_word =""
# last_committed_sign = None
# frame_since_hand_seen = 0

# start_time = time.perf_counter()
# last_timestamp = -1

# try:
#     with HandLandmarker.create_from_options(options)
# as landmarker:
#     while True:
#         success, frame = camrea.read()

#         if not success: 
#             print("Error! Could not read webcame frames. Please Try again!")
...
#         result = landmarker.dectect_for_video(mp_image, timestamp)
...
#             probabilites = static_moedl.predict_proba([features])[0]
#             best_index = np.argmax(probabilites)
...
#                 is_stable = (
#                     len(recent_predictions) == STABILITY_FRAMES and len(set(recent_predictions)) = 1
#                 )
...
#                 color = (0, 225, 0 ) if confidence >= CONFIDENCE_THRESHOLD else (0,0, 255)
...
#             cv2.putText(
#                 frame, "Press C tp clear word, Q to quit",
#                 (10, height - 15),
